...
class UnpairDataset(Dataset):
    def __init__(self,path: str,datatype: str = 'image',max_len: int = int(1e9),crop_size: Annotated[int, 'between [0,512]'] = 512,augment: bool = False) -> None:
        super().__init__()
        ...
        assert datatype in ['image', 'lmdb'], "datatype must be either 'image' or 'lmdb'."
        ...
        assert 0 <= crop_size <= 512 or crop_size == - 1, "crop_size must be between 0 and 512, or -1 if not used."
        ...
        transforms = [v2.ToImage(), v2.ToDtype(torch.float32, scale=True),v2.RandomCrop(crop_size)]
        if augment:
    ...
    def init_data(self, path: str) -> None:
        ...
        if self.datatype == 'image':
            ...
            self.input_keys = self.read_pkl(os.path.join(path, "meta_info.pkl"))['keys']
            self.get_data_func = _read
    ...
    def get_images(self,path: str,suffixs: list[str] = [".jpg",".JPG",".jpeg",".JPEG",".png",".PNG",".ppm",".PPM",".bmp",".BMP",],) -> list[str]:
        image_paths = []
    ...
    def init_lmdb_env(self, lmdb_path: str) -> lmdb.Environment:
        return lmdb.open(lmdb_path,readonly=True,lock=False,readahead=False,meminit=False,)

    def read_lmdb_by_key(self, key: str) -> Any:
        env = self.init_lmdb_env(self.path)
        with env.begin(write=False) as txn:
            return cv2.imdecode(np.frombuffer(txn.get(key.encode("ascii")),np.uint8), cv2.IMREAD_COLOR)
